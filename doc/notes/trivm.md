...
* Exported globals are accessible over structure that maps directly to VM instance.
  * Can be generated by `triconf`
  * Example
    ```c
    struct my_globals {
      uint32_t TMP0_reg;
      ...
      //...
      uint32_t ASPH_reg;
      uint32_t _padding0; // Globals area must be aligned.
      uint32_t my_global;
      uint32_t _padding1;
      uint64_t other_global;
    };

    static inline struct my_globals get_globals(trivm_instance* vm) {
      return (struct my_globals*)&vm->ram[0];
    }
    ```
