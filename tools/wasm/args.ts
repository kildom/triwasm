...
--optimize:OptLevel <level> = 2
    ...
    optimization, "s", "z" and "fast" are aliases for 2, "g" is alias for 1. The optimization
    strategy is always focused on size.

--vm-stack-size:VmStackSize <size> = 1024
    Set size of the virtual machine stack. Default is 1024. Special value 'shared' puts the
    virtual machine stack at the guest stack.

--global-base:size <address> = 0
    Address at which module starts storing data. Everything before that is unused by the
    module and can be used by the compiler and virtual machine. Required for memory size
    smaller than 64K. By default, it is 0.

--guest-stack-first
    Gest have its stack first, before '--global-base' address.

--guest-stack-guard
    Enable gest stack guard. Module must contain special function detecting the stack pointer
    global variable and virtual machine must have guest stack overflow or underflow
    fault enabled.

--guest-stack-size:size <size>
    Size of the guest stack. It is used only if '--guest-stack-first' or '--guest-stack-guard'
    options are enabled.
...
--debug-dump
    Dump various internal files alongside the output file. Use it to debug the compiler.
