...
import { getWasmConf, WasmArgs, WasmArgsMerge } from './args';
...
export function compileWasmModule(cmdLineArgs?: string[] | WasmArgs) {
    ...
    t.start();
    let conf = getWasmConf(cmdLineArgs);
    t.print('Load config');
    ...
    conf.args.output.withExtension('.pre.triasm').write(output.getOutput(false));
    let postProcessed = initTableGenerator.output.getOutput(false);
    conf.args.output.withExtension('.post.triasm').write(postProcessed);

    let asmCompiler = new asm.Compiler();
    let outputBinary = asmCompiler.compile(postProcessed);

    // TODOv1: Move it to triasm and make compiler option to enable generating of this file.
    let asmWithMap: string[] = [];
    for (let [lineNumber, line] of postProcessed.split('\n').entries()) {
        let address = asmCompiler.sourceMap[lineNumber + 1];
        if (typeof address === 'undefined') {
            asmWithMap.push(`               ${line}`.trimEnd());
        } else if (address < 0x80000000) {
            asmWithMap.push(`<% D${address.toString().padStart(7)} %> ${line}`.trimEnd());
        } else {
            asmWithMap.push(`<% P${(address - 0x80000000).toString().padStart(7)} %> ${line}`.trimEnd());
        }
    }

    conf.args.output.withExtension('.addr.triasm').write(asmWithMap.join('\n'));
